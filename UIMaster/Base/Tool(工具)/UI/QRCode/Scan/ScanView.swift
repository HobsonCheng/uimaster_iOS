...
//            dPrint(NSStringFromCGRect(cropRect))
    ...
    func drawScanRect() {
        ...
        if viewStyle.whRatio != 1.0 {
            ...
            var height: CGFloat = width / viewStyle.whRatio
...
//        dPrint("frame:%@",NSStringFromCGRect(self.frame))
...
//        右下角水平线
    ...
    static func getScanRectWithPreView(preView: UIView, style: ScanViewStyle) -> CGRect {
        ...
        if style.whRatio != 1 {
            let width = sizeRetangle.width
            var height = width / style.whRatio
        ...
        let p2: CGFloat = 1_920.0 / 1_080.0 //使用了1080p的图像输出
        if p1 < p2 {
            let fixHeight = size.width * 1_920.0 / 1_080.0
            ...
            let fixWidth = size.height * 1_080.0 / 1_920.0
    ...
    func deviceStartReadying(readyStr: String) {
        ...
        if (activityView == nil) {
            ...
            //dPrint("%@",NSStringFromCGRect(labelReadyRect))
