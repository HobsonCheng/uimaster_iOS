...
public struct  ScanResult {
 ...
 class ScanWrapper: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    init( videoPreView: UIView, objType: [AVMetadataObject.ObjectType] = [AVMetadataObject.ObjectType.qr], isCaptureImg: Bool, cropRect: CGRect = CGRect.zero, success:@escaping ( ([ScanResult]) -> Void) ) {
        do {
            if let deviceV = device {
                input = try AVCaptureDeviceInput(device: deviceV)
            }
        } catch let error as NSError {
            dPrint("AVCaptureDeviceInput(): \(error)")
        }
...
//        output.metadataObjectTypes = [AVMetadataObjectTypeQRCode,AVMetadataObjectTypeEAN13Code, AVMetadataObjectTypeEAN8Code, AVMetadataObjectTypeCode128Code]
        ...
        if device!.isFocusPointOfInterestSupported && device!.isFocusModeSupported(AVCaptureDevice.FocusMode.continuousAutoFocus) {
            do {
                ...
                dPrint("device.lockForConfiguration(): \(error)")
            }
        }
    }
...
//                dPrint("code type:%@",codeType)
                //码内容
                let codeContent = code.stringValue
//                dPrint("code string:%@",codeContent)

                //4个字典，分别 左上角-右上角-右下角-左下角的 坐标百分百，可以使用这个比例抠出码的图像
                // let arrayRatio = code.corners

                arrayResult.append(ScanResult(str: codeContent, img: UIImage(), barCodeType: codeType.rawValue, corner: code.corners as [AnyObject]?))
            }
        }
    ...
    // MARK: - ---拍照
     ...
     func connectionWithMediaType(mediaType: AVMediaType, connections: [AnyObject]) -> AVCaptureConnection? {
        for connection: AnyObject in connections {
            guard let connectionTmp: AVCaptureConnection = connection as? AVCaptureConnection else {
                return nil
            }

            for port: Any in connectionTmp.inputPorts {
                if (port as AnyObject).isKind(of: AVCaptureInput.Port.self) {
                    guard let portTmp: AVCaptureInput.Port = port as? AVCaptureInput.Port else {
                        continue
                    }
...
//        return floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_8_0
        ...
        if ScanWrapper.isSysIos8Later() {
            ...
            if features != nil && (features?.count)! > 0 {
                ...
                if feature.isKind(of: CIQRCodeFeature.self) {
                    guard let featureTmp: CIQRCodeFeature = feature as? CIQRCodeFeature else {
                        return []
                    }
    ...
    static  func getConcreteCodeRectFromImage(srcCodeImage: UIImage, codeResult: ScanResult) -> CGRect {
