...
extension UIAlertController {
    ...
    func addLocalePicker(type: LocalePickerViewController.Kind, selection: @escaping LocalePickerViewController.Selection) {
        ...
        let buttonSelect = UIAlertAction(title: "Select", style: .default) { _ in
            selection(info)
        }
...
final class LocalePickerViewController: UIViewController {
    ...
    func indexPathOfSelectedInfo() -> IndexPath? {
        guard let selectedInfo = selectedInfo else {
            return nil
        }
        if searchController.isActive {
...
extension LocalePickerViewController: UISearchResultsUpdating {
    func updateSearchResults(for searchController: UISearchController) {
        if let searchText = searchController.searchBar.text, searchController.isActive {
            filteredInfo = []
            if !searchText.isEmpty, let values = orderedInfo[String(searchText[searchText.startIndex])] {
                filteredInfo.append(contentsOf: values.filter { $0.country.hasPrefix(searchText) })
            } else {
                orderedInfo.forEach { _, value in
                    filteredInfo += value
                }
            }
            sortFilteredInfo()
        ...
        guard let selectedIndexPath = indexPathOfSelectedInfo() else {
            return
        }
...
extension LocalePickerViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        guard let info = info(at: indexPath) else {
            return
        }
        selectedInfo = info
...
extension LocalePickerViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        switch type {
        case .country:
            cell = tableView.dequeueReusableCell(withIdentifier: CountryTableViewCell.identifier) as? CountryTableViewCell ?? CountryTableViewCell()
            cell.textLabel?.text = info.country

        case .phoneCode:
            cell = tableView.dequeueReusableCell(withIdentifier: PhoneCodeTableViewCell.identifier) as? PhoneCodeTableViewCell ?? PhoneCodeTableViewCell()
            cell.textLabel?.text = info.phoneCode
            cell.detailTextLabel?.text = info.country

        case .currency:
            cell = tableView.dequeueReusableCell(withIdentifier: CurrencyTableViewCell.identifier) as? CurrencyTableViewCell ?? CurrencyTableViewCell()
