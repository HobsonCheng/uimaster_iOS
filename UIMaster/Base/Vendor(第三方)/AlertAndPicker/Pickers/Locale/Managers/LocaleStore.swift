...
struct LocaleStore {
    ...
    public static func getInfo(completionHandler: @escaping (FetchResults) -> Void) {
        ...
        if let jsonObjects = (try? JSONSerialization.jsonObject(with: jsonData, options: JSONSerialization.ReadingOptions.allowFragments)) as? Array<Any> {
            ...
            for jsonObject in jsonObjects {
                guard let countryObj = jsonObject as? [String: Any] else { continue }
    ...
    public static func fetch(completionHandler: @escaping (GroupedByAlphabetsFetchResults) -> Void) {
