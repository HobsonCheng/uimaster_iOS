...
extension UIAlertController {
    ...
    func addPhotoLibraryPicker(flow: UICollectionViewScrollDirection, paging: Bool, selection: PhotoLibraryPickerVC.Selection) {
        let selection: PhotoLibraryPickerVC.Selection = selection
        var asset: PHAsset?
        var assets: [PHAsset] = []

        let buttonAdd = UIAlertAction(title: "添加", style: .default) { action in
            switch selection {
        ...
        let vc = PhotoLibraryPickerVC(flow: flow, paging: paging, selection: {
            switch selection {
            case .single:
            ...
            case .multiple:
                return .multiple(action: { new in
                    buttonAdd.isEnabled = new.isEmpty
                    assets = new
                })
            }
        }()
...
final class PhotoLibraryPickerVC: UIViewController {
    ...
    required init(flow: UICollectionViewScrollDirection, paging: Bool, selection: Selection) {
    ...
    func checkStatus(completionHandler: @escaping ([PHAsset]) -> Void) {
        ...
        case .notDetermined:
            ...
            PhotoAssets.requestAccess { [unowned self] _ in
    ...
    func fetchPhotos(completionHandler: @escaping ([PHAsset]) -> Void) {
        PhotoAssets.fetch { [unowned self] result in
            switch result {
...
extension PhotoLibraryPickerVC: UICollectionViewDelegate {
...
extension PhotoLibraryPickerVC: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let item = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: ItemWithImage.self), for: indexPath) as? ItemWithImage else {
            return UICollectionViewCell()
        }
        let asset = assets[indexPath.item]
        PhotoAssets.resolve(asset: asset, size: item.bounds.size) { new in
...
extension PhotoLibraryPickerVC: UICollectionViewDelegateFlowLayout {
