...
typealias TelegramSelection = (TelegramSelectionType) -> Void
...
final class TelegramPickerViewController: UIViewController {
    ...
    func title(for button: ButtonType) -> String {
        ...
        case .contact:
            return "Contact"
        case .sendPhotos:
            return "Send \(selectedAssets.count) \(selectedAssets.count == 1 ? "Photo" : "Photos")"
        case .sendAsFile:
            return "Send as File"
        }
    }
    ...
    func sizeFor(asset: PHAsset) -> CGSize {
        ...
        let width = CGFloat(Double(height) * Double(asset.pixelWidth) / Double(asset.pixelHeight))
...
//        $0.contentInsetAdjustmentBehavior = .never
    ...
    func checkStatus(completionHandler: @escaping ([PHAsset]) -> Void) {
        aLog("status = \(PHPhotoLibrary.authorizationStatus())")
...
extension TelegramPickerViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let item = collectionView.dequeueReusableCell(withReuseIdentifier: String(describing: ItemWithPhoto.self), for: indexPath) as? ItemWithPhoto else {
        ...
        DispatchQueue.main.async {
            PhotoAssets.resolve(asset: asset, size: size) { new in
...
extension TelegramPickerViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: LikeButtonCell.identifier) as? LikeButtonCell else {
