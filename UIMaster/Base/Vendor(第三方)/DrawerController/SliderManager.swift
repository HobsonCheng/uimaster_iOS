...
public class SliderManager: NSObject {
    unowned let drawer: DrawerViewController
    ...
    private(set) var slider: NaviBarVC? {
        willSet {
    ...
    internal init(drawer: DrawerViewController) {
        self.drawer = drawer
        super.init()
    }

    func addSlider(slider: NaviBarVC ,
...
//        slider.endAppearanceTransition()
    ...
    fileprivate func setSliderLayout() {
        guard slider?.view.mLayout.getConstraint(attr: NSLayoutAttribute.leading) == nil && slider?.view.mLayout.getConstraint(attr: NSLayoutAttribute.leading) == nil else {
            return
        }
    ...
    fileprivate func isSliderFront() -> Bool {
        switch self.mode {
        case .frontWidth, .frontWidthRate:
            return true
        default:
            return false
        }
    ...
    fileprivate func setShiftWidth(drawSize: CGSize) {
        ...
        switch mode {
        case .frontWidth(let frontW):
            shiftWidth = frontW
        case .frontWidthRate(let frontWRate):
            shiftWidth = mainW * frontWRate
        case .rearWidth(let rearW):
            shiftWidth = rearW
        case .rearWidthRate(let rearWRate):
            shiftWidth = mainW * rearWRate
        default: break
        }
    }
