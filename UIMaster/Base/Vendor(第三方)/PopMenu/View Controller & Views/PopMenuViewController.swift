...
public final class PopMenuViewController: UIViewController {
    // MARK: - Properties
...
//        containerView.addShadow(offset: .init(width: 0, height: 1), opacity: 0.5, radius: 20)
        ...
        containerView.layer.borderWidth = 0.35
...
extension PopMenuViewController {
    ...
    @objc fileprivate func menuDidPan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .began, .changed:
            if let index = associatedActionIndex(gesture) {
                ...
                actions.filter { !$0.isEqual(action) }.forEach { $0.highlighted = false }
            }
        case .ended:
            // Unhighlight all actions.
            actions.filter { $0.highlighted }.forEach { $0.highlighted = false }
    ...
    fileprivate func associatedActionIndex(_ gesture: UIGestureRecognizer) -> Int? {
        ...
        if let touchedView = actionsView.arrangedSubviews.filter({ $0.frame.contains(touchLocation) }).first,
