...
private let TGActionSheetCancelTag = 1_999
private let TGActionSheetBaseTag = 1_000
private let TGActionSheetAnimationDuration: TimeInterval = 0.25
...
class TGActionSheet: UIView {
    ...
    init(delegate: TGActionSheetDelegate?, title: String? = nil, cancelTitle: String, otherTitles: [String]) {
        ...
        for idx in 0..<otherTitles.count {
            self.createBtn(with: otherTitles[idx], bgColor: UIColor(white: 1, alpha: 1), titleColor: .darkGray, tagIndex: idx + TGActionSheetBaseTag)
        }
    ...
    func show() {
        ...
        let btnX: CGFloat = 0
        var btnY: CGFloat = 0
        for idx in 0..<btnArr.count {
            let btn = btnArr[idx]
            let divider = dividerArr[idx]
            btnY = TGPhotoPickerConfig.shared.toolBarH * CGFloat(idx)
            btn.frame = CGRect(x: btnX, y: btnY, width: btnW, height: btnH)
            divider.frame = CGRect(x: btnX, y: btnH - 1, width: btnW, height: 1)
        }
        self.actionSheet.height += kiPhoneXBottomH
