...
protocol TGPhotoCollectionDelegate: AnyObject {
...
class TGAlbumPhotoPreviewVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if TGPhotoPickerConfig.shared.isShowIndicator && (TGPhotoPickerConfig.shared.indicatorPosition != .inBottomBar) {
            ...
            case .top:
                ...
                indicatorLabel?.origin = CGPoint(x: (TGPhotoPickerConfig.ScreenW - (bottomBar?.indicatorLabel.w)!) / 2, y: (topBar?.frame.maxY)! - (indicatorLabel?.height)! + 5)
            case .bottom:
                self.view.addSubview(indicatorLabel!)
                indicatorLabel?.origin = CGPoint(x: (TGPhotoPickerConfig.ScreenW - (bottomBar?.indicatorLabel.w)!) / 2, y: (bottomBar?.y)! - (indicatorLabel?.height)! - 5)
            default:
                self.topBar?.addSubview(indicatorLabel!)
                indicatorLabel?.origin = CGPoint(x: (TGPhotoPickerConfig.ScreenW - (bottomBar?.indicatorLabel.w)!) / 2, y: ((topBar?.height)! - (indicatorLabel?.height)!) / 2)
            }
    ...
    private func setupBar() {
        ...
        topBar?.nav = self.nav
        topBar?.selectNum = self.nav?.alreadySelectedImageNum ?? 0
        ...
        self.bottomBar?.changeNumber(number: nav?.assetArr.count ?? 0, animation: false)
...
extension TGAlbumPhotoPreviewVC: TGTopBarDelegate {
    ...
    func onSelectedClicked(select: Bool) {
        ...
        self.bottomBar?.changeNumber(number: self.nav?.assetArr.count ?? 0, animation: TGPhotoPickerConfig.shared.checkboxAnimate)
    }
}
...
extension TGAlbumPhotoPreviewVC: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellID, for: indexPath) as? TGPhotoPreviewCell else {
    ...
    func onImageSingleTap() {
        ...
        if self.topBar!.frame.origin.y < 0 {
            UIView.animate(withDuration: 0.3, delay: 0, options: [UIViewAnimationOptions.curveEaseOut], animations: {
                ...
                originPoint.y -= self.bottomBar!.frame.height
