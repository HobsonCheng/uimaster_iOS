...
@objc
protocol TGBottomBarDelegate: AnyObject {
...
class TGBottomBar: UIView {
        ...
        didSet {
            if host == ("\(type(of: TGPhotoCollectionVC.self))" as NSString).components(separatedBy: ".").first! {
                ...
                for idx in 0..<self.subviews.count {
                    if subviews[idx].isKind(of: TGAnimationButton.self) {
                        if prevBtn == nil {
                            subviews[idx].x = padding
                            prevBtn = subviews[idx] as? TGAnimationButton
                        } else {
                            subviews[idx].x = (prevBtn?.rightX)! + padding
                            prevBtn = subviews[idx] as? TGAnimationButton
                        }
                    }
                }
                if TGPhotoPickerConfig.shared.isShowIndicator {
                    ...
                    let num = ((doneButton?.left ?? self.w) - (prevBtn?.rightX ?? 0) - indicatorLabel.w) / 2
                    indicatorLabel.x = (prevBtn?.rightX ?? 0) + num
                }
        ...
        */
        ...
        if TGPhotoPickerConfig.shared.isShowIndicator {
            ...
            attributeString.addAttribute(NSAttributedStringKey.font,
                                         ...
                                         range: NSRange(location: "\(number) ".characters.count, length: 1))

            attributeString.addAttribute(NSAttributedStringKey.font,
                                         value: UIFont.systemFont(ofSize: TGPhotoPickerConfig.shared.fontSize - 3),
                                         range: NSRange(location: "\(number) / ".characters.count, length: "\(TGPhotoPickerConfig.shared.maxImageCount)".characters.count))
    ...
    lazy var indicatorLabel: UILabel = {
        ...
        indicatorLbl.height = TGPhotoPickerConfig.shared.toolBarH * 0.8
    ...
    private lazy var previewButton: TGAnimationButton = {
        ...
        previewBtn.height = TGPhotoPickerConfig.shared.doneButtonH * 0.9
    ...
    private lazy var reselectButton: TGAnimationButton = {
        ...
        reselectBtn.height = TGPhotoPickerConfig.shared.doneButtonH * 0.9
    ...
    private lazy var originalButton: TGAnimationButton = {
        ...
        originalBtn.height = TGPhotoPickerConfig.shared.doneButtonH * 0.9
    ...
    private lazy var editButton: TGAnimationButton = {
        ...
        editBtn.sizeToFit()
        editBtn.height = TGPhotoPickerConfig.shared.doneButtonH * 0.9
