...
@available(iOS 10.0, *)
class TGCameraVC: BaseNameVC {
    var callbackPicutureData: ((Data?) -> Void)?
...
extension TGCameraVC: AVCapturePhotoCaptureDelegate {
    func photoOutput(_ captureOutput: AVCapturePhotoOutput, didFinishProcessingPhoto photoSampleBuffer: CMSampleBuffer?, previewPhoto previewPhotoSampleBuffer: CMSampleBuffer?, resolvedSettings: AVCaptureResolvedPhotoSettings, bracketSettings: AVCaptureBracketedStillImageSettings?, error: Error?) {
        if error != nil {
            dPrint("error = \(String(describing: error?.localizedDescription))")
        } else {
...
//                let alert =  UIAlertView(title: TGPhotoPickerConfig.shared.saveImageTip, message: msg, delegate: self, cancelButtonTitle: TGPhotoPickerConfig.shared.confirmTitle)
                let alertVC = UIAlertController(title: TGPhotoPickerConfig.shared.saveImageTip, message: msg, preferredStyle: .alert)
                alertVC.addAction(UIAlertAction(title: TGPhotoPickerConfig.shared.confirmTitle, style: .cancel, handler: nil))
                kWindowRootVC?.present(alertVC, animated: true, completion: nil)
//                alert.show()
            }
        }
    ...
    fileprivate func canUseAlbum(returnClosure:@escaping (Bool) -> Void) {
