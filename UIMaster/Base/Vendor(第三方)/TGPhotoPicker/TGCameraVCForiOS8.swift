...
class TGCameraVCForiOS8: BaseNameVC {
    ...
    fileprivate lazy var imageOutput = AVCaptureStillImageOutput()
    ...
    fileprivate func canUseCamera(returnClosure:@escaping (Bool) -> Void) {
    ...
    @objc fileprivate func takePhotoAction() {
        ...
        imageOutput.captureStillImageAsynchronously(from: videoConnection) { imageDataSampleBuffer, error in
            if error != nil {
                ...
                dPrint("image size\(String(describing: self.image?.size))")
            }
        }
    }
