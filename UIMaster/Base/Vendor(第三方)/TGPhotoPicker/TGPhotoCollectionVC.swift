...
protocol TGPhotoCollectionViewCellDelegate: AnyObject {
...
class TGPhotoCell: UICollectionViewCell {
    ...
    private func getCacheImage(_ index: Int) -> UIImage? {
        if !TGPhotoPickerConfig.shared.cacheNumerImageArr.isEmpty &&
            index >= 0 &&
...
class TGPhotoCollectionVC: UICollectionViewController {
    ...
    fileprivate lazy var assetGridThumbnailSize: CGSize = {
        ...
        let size = (cellSize?.width ?? 0) * UIScreen.main.scale
    ...
    fileprivate lazy var bottomBar: TGBottomBar = {
        ...
        if !(self.nav?.assetArr.isEmpty ?? true) {
            toolbar.changeNumber(number: self.nav?.assetArr.count ?? 0, animation: false)
        }
        return toolbar
    }()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        selectNumberChange(number: self.nav?.assetArr.count ?? 0)
    }
    ...
    private func setupUI() {
        ...
        resetCacheAssets()
        self.collectionView?.contentInset = UIEdgeInsets(top: TGPhotoPickerConfig.shared.padding, left: TGPhotoPickerConfig.shared.leftAndRigthNoPadding ? 0 : TGPhotoPickerConfig.shared.padding, bottom: TGPhotoPickerConfig.shared.padding + ((TGPhotoPickerConfig.shared.barBGColor.getAlpha() != 1) ? TGPhotoPickerConfig.shared.toolBarH : 0), right: TGPhotoPickerConfig.shared.leftAndRigthNoPadding ? 0 : TGPhotoPickerConfig.shared.padding)
...
//                context.addLine(to: CGPoint(x: 12, y: 16))
    ...
    fileprivate func setupCell(_ cell: TGPhotoCell, _ asset: PHAsset) {
        ...
        cell.isMaskHidden = (cell.photoSelectedIndex > -1) ? true : (TGPhotoPickerConfig.shared.useSelectMask ? true : !((nav?.assetArr.count ?? 0) >= TGPhotoPickerConfig.shared.maxImageCount))
        ...
        self.imageManager.requestImage(for: asset, targetSize: self.assetGridThumbnailSize, contentMode: .aspectFill, options: nil) { image, _ in
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as? TGPhotoCell else {
...
extension TGPhotoCollectionVC: TGBottomBarDelegate {
    ...
    func onPreviewButtonClicked(_ sender: TGAnimationButton) {
        let previewvc = TGAlbumPhotoPreviewVC()
        previewvc.fetchResult = self.fetchResult
        if let asset = nav?.assetArr.first {
            previewvc.currentPage = (self.fetchResult?.index(of: asset) ?? 0)
        }
        previewvc.delegate = self
        self.navigationController?.show(previewvc, sender: nil)
    }
    ...
    func onReselectButtonClicked(_ sender: TGAnimationButton) {
        ...
        selectNumberChange(number: (nav?.assetArr.count ?? 0), forceRefresh: true)
    }
}
...
extension TGPhotoCollectionVC: TGPhotoCollectionDelegate {
    func onPreviewPageBack() {
        self.collectionView?.reloadData()
        self.selectNumberChange(number: self.nav?.assetArr.count ?? 0)
    }
}
        ...
        if forceRefresh ||
            ...
            UIView.performWithoutAnimation {
                self.collectionView?.performBatchUpdates({
                    ...
                    for idx in 0 ..< (self.collectionView?.visibleCells.count ?? 0) {
                        let cell = self.collectionView?.visibleCells[idx] as? TGPhotoCell ?? TGPhotoCell()
