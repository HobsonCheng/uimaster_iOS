    ...
    func renderData(_ result: PHFetchResult<AnyObject>, label: String?) {
        ...
        if result.count > 0 {
            if let firstImageAsset = result[0] as? PHAsset {
                ...
                PHImageManager.default().requestImage(for: firstImageAsset, targetSize: size, contentMode: .aspectFill, options: imageOptions, resultHandler: { image, _ in
                    self.coverImage.image = image
                })
            }
        }
    }
    // swiftlint:enable Empty Count
...
//        }
    ...
    @objc private func navDismiss() {
        guard let nav = self.navigationController as? TGPhotoPickerVC else {
            return
        }
    ...
    fileprivate func loadAlbums(_ replace: Bool) {
        ...
        dPrint("smartAlbums.count:\(smartAlbums.count)")
        for idx in 0 ..< smartAlbums.count {//14 (contains `Recently Deleted`)
            if TGPhotoPickerConfig.shared.useCustomSmartCollectionsMask {
                if TGPhotoPickerConfig.shared.customSmartCollections.contains(smartAlbums[idx].assetCollectionSubtype) {
                    self.filterFetchResult(collection: smartAlbums[idx])
                }
            } else {
                self.filterFetchResult(collection: smartAlbums[idx])
            }
        ...
        for idx in 0 ..< topUserLibraryList.count {
            if let topUserAlbumItem = topUserLibraryList[idx] as? PHAssetCollection {
    ...
    private func filterFetchResult(collection: PHAssetCollection) {
        ...
        dPrint("\(String(describing: collection.localizedTitle)):\(fetchResult.count)")

        guard let result = fetchResult as? PHFetchResult<PHObject> else {
            return
        }
        if TGPhotoPickerConfig.shared.isShowEmptyAlbum {
            self.albums.append(TGFetchM(result: result, name: collection.localizedTitle, assetType: collection.assetCollectionSubtype))
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? TGPhotoListCell else {
