...
import UIKit
protocol TGPhotoPickerCellDelegate: AnyObject {
...
class TGPhotoPicker: UIView {
    ...
    fileprivate weak var vc: BaseNameVC?
    weak var collectionView: UICollectionView?
    lazy var config = TGPhotoPickerConfig.shared
 ...
 */
    ...
    init(_ vc: BaseNameVC, frame: CGRect, _ configBlock:((_ config: TGPhotoPickerConfig) -> Void)? = nil) {
    ...
    class func photoPicker(_ vc: BaseNameVC, frame: CGRect, _ configBlock:((_ config: TGPhotoPickerConfig) -> Void)? = nil) -> TGPhotoPicker {
    ...
    private func createCollectionView() {
        ...
        cv.delegate = self
        cv.dataSource = self
        cv.contentInset = UIEdgeInsets(top: config.padding, left: config.leftAndRigthNoPadding ? 0 : config.padding, bottom: config.padding, right: self.bounds.width - config.mainColCount * config.mainCellWH - (config.mainColCount + (config.leftAndRigthNoPadding ? -1 : 0)) * config.padding)
...
extension TGPhotoPicker: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: reuseIdentifier, for: indexPath) as? TGPickerCell else {
    ...
    func remove(_ model: TGPhotoM?) {
        ...
        if self.tgphotos.count == TGPhotoPickerConfig.shared.maxImageCount {//删除最大张数时的最后一张或中间一张
            DispatchQueue.main.async {
                let cell = self.collectionView?.cellForItem(at: IndexPath(row: index, section: 0)) as? TGPickerCell
                cell?.photoM = nil
...
extension TGPhotoPicker: UICollectionViewDelegate, TZImagePickerControllerDelegate {
 ...
 */
    ...
    fileprivate func addPhotos() {
        let imagePickerVC = TZImagePickerController(maxImagesCount: TGPhotoPickerConfig.shared.maxImageCount - self.tgphotos.count, delegate: self)
        imagePickerVC?.allowPickingVideo = false
        imagePickerVC?.didFinishPickingPhotosHandle = {[weak self] photos, assets, finish in
            var tgPhotoMArr: [TGPhotoM] = []
            for photo in photos ?? [] {
                let image = TGPhotoM()
                image.bigImage = photo
                image.smallImage = photo
                tgPhotoMArr.append(image)
            }
            self?.tgphotos.append(tgPhotoMArr)
            DispatchQueue.main.async {
                self?.collectionView?.reloadData()
            }
        }
...
//        }
    ...
    private func previewPhoto(_ index: Int) {
        ...
        kWindowRootVC?.present(nav, animated: false, completion: nil)
    }
}
    ...
    func actionSheet(actionSheet: TGActionSheet?, didClickedAt index: Int) {
        ...
        case 0:
            if TGPhotoPickerConfig.shared.useiOS8Camera {
                ...
                VCController.push(cameraVC, with: VCAnimationBottom.defaultAnimation())
...
//    func onImageSelectFinished(images: [UIImage?]) {
//
//        self.tgphotos.append(contentsOf: newArr)
//    }
//    func onImageSelectFinished(images: [PHAsset]) {
...
class TGPickerCell: UICollectionViewCell {
    ...
    private lazy var addImage: UIImage = {
        let cellWH = TGPhotoPickerConfig.shared.mainCellWH
        return UIImage.size(width: cellWH, height: cellWH)
            .color(.clear)
            .border(color: .clear)
            ...
            .with({ context in
                ...
                context.move(to: CGPoint(x: cellWH * 0.25, y: cellWH * 0.5))
                context.addLine(to: CGPoint(x: cellWH * 0.75, y: cellWH * 0.5))
                context.move(to: CGPoint(x: cellWH * 0.5, y: cellWH * 0.25))
                context.addLine(to: CGPoint(x: cellWH * 0.5, y: cellWH * 0.75))
                context.strokePath()
                ...
                context.addLine(to: CGPoint(x: cellWH - 1, y: 1))
                context.move(to: CGPoint(x: cellWH - 1, y: 1))
                context.addLine(to: CGPoint(x: cellWH - 1, y: cellWH - 1))
                context.move(to: CGPoint(x: cellWH - 1, y: cellWH - 1))
                context.addLine(to: CGPoint(x: 1, y: cellWH - 1))
                context.move(to: CGPoint(x: 1, y: cellWH - 1))
