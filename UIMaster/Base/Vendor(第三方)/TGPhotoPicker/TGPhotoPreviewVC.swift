...
protocol TGPhotoPreviewDelegate: AnyObject {
...
//                context.addLine(to: CGPoint(x: 12, y: 16))
    ...
    private func setupCollectionView() {
        ...
        case .bottom, .inBottomBar:
            self.view.addSubview(indicatorLabel)
            indicatorLabel.x = (TGPhotoPickerConfig.ScreenW - indicatorLabel.w) / 2
            indicatorLabel.y = self.view.height - indicatorLabel.height - 5
        }
    }
    ...
    fileprivate func updatePageTitle() {
        ...
        attributeString.addAttribute(NSAttributedStringKey.font,
                                     ...
                                     range: NSRange(location: 0, length: "\(self.currentPage + 1) ".characters.count))

        attributeString.addAttribute(NSAttributedStringKey.font,
                                     value: UIFont.boldSystemFont(ofSize: TGPhotoPickerConfig.shared.fontSize),
                                     range: NSRange(location: "\(self.currentPage + 1) ".characters.count, length: 1))

        attributeString.addAttribute(NSAttributedStringKey.font,
                                     value: UIFont.systemFont(ofSize: TGPhotoPickerConfig.shared.fontSize - 3),
                                     range: NSRange(location: "\(self.currentPage + 1) / ".characters.count, length: "\(self.selectImages.count)".characters.count))
...
extension TGPhotoPreviewVC: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        guard let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellIdentifier, for: indexPath) as? TGPhotoPreviewCell else {
