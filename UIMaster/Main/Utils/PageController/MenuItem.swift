...
class MenuItem: UILabel {
    ...
    var normalColor: UIColor? {
        didSet {
            ...
            if normalColor?.getRed(&fRed, green: &fGreen, blue: &fBlue, alpha: &fAlpha) ?? false {
                normalComponents.red = fRed
                normalComponents.green = fGreen
                normalComponents.blue = fBlue
                normalComponents.alpha = fAlpha
            }
        }
    }
    ...
    var selectedColor: UIColor? {
        didSet {
            ...
            var fBlue: CGFloat = 0
            var fAlpha: CGFloat = 0
            if selectedColor?.getRed(&fRed, green: &fGreen, blue: &fBlue, alpha: &fAlpha) ?? false {
                selectedComponets.red = fRed
                selectedComponets.green = fGreen
                selectedComponets.blue = fBlue
                selectedComponets.alpha = fAlpha
            }
//            selectedColor?.getRed(&selectedComponets.red, green: &selectedComponets.green, blue: &selectedComponets.blue, alpha: &selectedComponets.alpha)
    ...
    // MARK: - Public funcs
    func selectWithAnimation(_ select: Bool) {
